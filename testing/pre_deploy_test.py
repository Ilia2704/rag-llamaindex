...
OLLAMA_HEALTH_URL = (
    OLLAMA_BASE_URL[:-3] + "/api/tags"
    if OLLAMA_BASE_URL.endswith("/v1")
    else OLLAMA_BASE_URL.rstrip("/") + "/api/tags"
)
...
def get_model_runs() -> list[dict[str, str]]:
    ...
    runs = [dict(item) for item in MODEL_RUNS]
    for model_cfg in runs:
        if model_cfg["provider"] == "yandex" and not model_cfg["model"]:
            model_cfg["model"] = _default_yandex_model()
...
def ensure_mlflow_experiment() -> None:
    base_name = (os.getenv("MLFLOW_EXPERIMENT_NAME") or "rag-engineering-workshop").strip()
...
def run_model_test(model_cfg: dict[str, str]) -> bool:
    if model_cfg["provider"] == "yandex" and not model_cfg["model"]:
        raise RuntimeError("Для модели Yandex нужно заполнить YC_FOLDER_ID или OPENAI_MODEL в .env.")
    ...
    env.update(
        {
            ...
            "MLFLOW_RUN_NAME": f"ragas_robotex::{model_cfg['name']}",
            "RAGAS_REFERENCE_FREE": (os.getenv("RAGAS_REFERENCE_FREE") or "0").strip(),
        }
    )
            ...
            mlflow.log_params(
                {
                    ...
                    "eval_model": payload.get("eval_model", ""),
                        ...
                        "reference_free": str(payload.get("reference_free", "")),
                    }
                )
            mlflow.set_tags(
                {
...
def main() -> None:
    ...
    try:
        with mlflow.start_run(run_name="pre_deploy_test"):
            mlflow.set_tags({"suite": "pre_deploy_test", "project": "rag-engineering-workshop"})
